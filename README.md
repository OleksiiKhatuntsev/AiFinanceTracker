...
## AI Service: Python
#### Decision: 
Python (FastAPI / LangChain) as a dedicated microservice.
...
## Data Storage: "Dual Memory" Strategy
#### Decision: 
...
- Solves the "LLM Math Hallucination" problem. Vector DBs cannot perform accurate aggregate functions (e.g., SUM), so precise figures are retrieved via SQL, while semantic context is retrieved via vector search.

## Inter-service Communication: Ports & Adapters
#### Decision: 
REST API (HTTP) for MVP, with strictly defined C# Interfaces (Abstractions).
...
## Infrastructure: On-Premises (Localhost) -> Cloud Ready
#### Decision: 
...
#### Decision: 
Implement MCP to connect tools to the LLM.

#### Justification: 

Standardization. Instead of writing custom "glue code" for function calling, MCP provides a unified protocol. This makes the architecture extensible and allows for easy integration of pre-built tools (e.g., Google Drive, Slack).

## Data Processing Strategy: Rule-Based First
#### Decision: 
Apply deterministic rules (Regex/Hardcoded logic) for known transaction types (e.g., salary, internal transfers) before falling back to AI.
